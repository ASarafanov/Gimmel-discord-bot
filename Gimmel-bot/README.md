...
- Чтобы дни начали/сбросились, пользователь должен зайти в voice/stage канал или перейти между voice/stage каналами.
- Обычное сообщение в текстовом канале не обновляет `last_seen` в текущей версии.
- Дни считаются как разница календарных дат в `timezone` сервера.
